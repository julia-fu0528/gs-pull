...
class SuGaR(nn.Module):
    ...
    def marching_cubes_part(self, iteration=0, checkpoint_path='.', resolution=256, vertex_color=False, thres=0., 
                            part=1, world_space=True, crop_border=True, delta=0.0):
    ...
    def marching_cubes_thick_part(
        ...
        """
        Extract a thick cloth surface by combining:
            - outer surface: SDF = +thickness
            - inner surface: SDF = -thickness

        Produces a watertight 2-manifold mesh even for very thin cloth.
        """
        ...
        # -------------------------------------------------------
        # 1. Determine bounding box in *normalized space*
        # -------------------------------------------------------
        all_pts_norm = (self.points - dataset.shape_center) / dataset.shape_scale

        block_min_norm = (torch.tensor(dataset.block_min, device=self.device) - dataset.shape_center) / dataset.shape_scale
        block_max_norm = (torch.tensor(dataset.block_max, device=self.device) - dataset.shape_center) / dataset.shape_scale

        pad = (torch.tensor(dataset.split_size, device=self.device) / dataset.shape_scale) / 20
        block_min_norm -= pad
        block_max_norm += pad

        part_mask = torch.all(
            (all_pts_norm >= block_min_norm) &
            (all_pts_norm <= block_max_norm),
            dim=1
        )

        if part_mask.sum() > 0:
            pts = all_pts_norm[part_mask]
            actual_min = pts.min(0)[0].cpu().numpy() - 0.05
            actual_max = pts.max(0)[0].cpu().numpy() + 0.05
            actual_min = np.clip(actual_min, -1, 1)
            actual_max = np.clip(actual_max, -1, 1)
        else:
            actual_min = dataset.object_bbox_min
            actual_max = dataset.object_bbox_max
        ...
        # -------------------------------------------------------
        # 2. Sample NEUS SDF on a uniform grid
        # -------------------------------------------------------
        N = 64  # chunk size for memory
        X = torch.linspace(bound_min[0], bound_max[0], resolution).split(N)
        Y = torch.linspace(bound_min[1], bound_max[1], resolution).split(N)
        Z = torch.linspace(bound_min[2], bound_max[2], resolution).split(N)
            ...
            for xi, xs in enumerate(X):
                for yi, ys in enumerate(Y):
                    for zi, zs in enumerate(Z):
                        ...
                        pts = torch.stack([xx, yy, zz], -1).reshape(-1, 3).cuda()

                        sdf_vals = sdf_network.sdf(pts)
                        sdf_vals = sdf_vals.reshape(len(xs), len(ys), len(zs)).cpu().numpy()
        ...
        # -------------------------------------------------------
        # 3. Extract positive and negative level sets
        # -------------------------------------------------------
        bmin = bound_min.cpu().numpy()
        bmax = bound_max.cpu().numpy()
        scale = (bmax - bmin)

        # OUTER = SDF + thickness
        verts_o, faces_o = mcubes.marching_cubes(u, +thickness)
        if verts_o.size == 0:
            print(f"[WARN] No outer surface extracted for part {part} at +{thickness}")
        else:
            verts_o = verts_o / (resolution - 1.0) * scale + bmin

        # INNER = SDF - thickness
        verts_i, faces_i = mcubes.marching_cubes(u, -thickness)
        if verts_i.size == 0:
            print(f"[WARN] No inner surface extracted for part {part} at -{thickness}")
        else:
            verts_i = verts_i / (resolution - 1.0) * scale + bmin

        # -------------------------------------------------------
        # 4. Convert to world space
        # -------------------------------------------------------
        if world_space:
            verts_o = verts_o * dataset.shape_scale.cpu().numpy() + dataset.shape_center.cpu().numpy()
            verts_i = verts_i * dataset.shape_scale.cpu().numpy() + dataset.shape_center.cpu().numpy()

        # -------------------------------------------------------
        # 5. Fix triangle indexing + flip inner surface
        # -------------------------------------------------------
        offset = len(verts_o)
        faces_i = faces_i + offset
        faces_i = np.flip(faces_i, axis=1)

        # -------------------------------------------------------
        # 6. Merge geometry
        # -------------------------------------------------------
        verts = np.vstack([verts_o, verts_i])
        faces = np.vstack([faces_o, faces_i])

        # -------------------------------------------------------
        # 7. Create mesh and clean
        # -------------------------------------------------------
        ...
        # -------------------------------------------------------
        print("\n========== [Thick MC Debug] ==========")
        print(f"Part {part} | thickness = {thickness}")
        print("Outer verts:", len(verts_o), "faces:", len(faces_o))
        print("Inner verts:", len(verts_i), "faces:", len(faces_i))
        print("Merged verts:", len(verts), "faces:", len(faces))
        print("======================================\n")

        # -------------------------------------------------------
        # 9. Save mesh
        # -------------------------------------------------------
        save_dir = os.path.join(checkpoint_path, "meshes")
        os.makedirs(save_dir, exist_ok=True)

        out_path = os.path.join(save_dir, f"mcubes_thick_{iteration}_{part}.ply")
        mesh.export(out_path)

        print(f"[Thick MC] Saved → {out_path}")
